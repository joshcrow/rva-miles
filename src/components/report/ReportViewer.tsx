...
export function ReportViewer() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setState({
        kind: "error",
        // decodeReport writes for the manager; anything else is internal.
        message: isUserFacing(err) ? err.message : "This report link can't be read.",
      });
    }
  }, []);
  ...
  useEffect(() => {
    ...
    // Becomes the default filename when the manager prints this to PDF, so it
    // names the driver the same way the spreadsheet does.
    const who = state.payload.ownerName ? ` — ${state.payload.ownerName}` : "";
    document.title = `${state.payload.title}${who} — ${rangeLabelLong(state.payload.range)}`;
    return () => {
      ...
      <Box className="rva-report-viewer" sx={{ width: "100%" }}>
        ...
        >
          ...
          <Typography variant="caption" color="text.secondary" sx={{ display: "block", mt: 1.5 }}>
            {/* This sentence has to survive being checked by hand with a
                calculator, including on a trip whose distance was typed to two
                decimals, so it names the rounding rather than promising an
                identity the printed column can't always satisfy. */}
            {`${pluralTrips(totals.count)} · each amount is that trip's distance × the rate in effect on its date, and miles are shown to one decimal.`}
          </Typography>
